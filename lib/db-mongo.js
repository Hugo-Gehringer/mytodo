...
function DB(credentials) {
  const DB_NAME = 'mytodo-gehringerhugo-1816';
  ...
  self.init = () => {
    return new Promise((resolve, reject) => {
      ...
      // Chemin vers le fichier PEM à la racine du projet
      const pemFilePath = path.join(__dirname, 'ca.pem');

// Lire le contenu du fichier PEM
      const pemCertificate = fs.readFileSync(pemFilePath, 'utf8');

// Encoder le contenu en base64
      const ca = Buffer.from(pemCertificate).toString('base64');
      ...
      let connectionString = `mongodb://${username}:${password}@${connectionPath}/?replicaSet=replset`;
      ...
      var options = {
        ...
        sslValidate: true,
        sslCA: ca,
        tlsAllowInvalidCertificates: true, // Ajout pour accepter les certificats auto-signés
        poolSize: 1,
        useNewUrlParser: true,
        useUnifiedTopology: true
      };
      // MongoClient.connect(credentials.MONGO_URI, options,  {
